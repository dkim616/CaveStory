...
PolarStar::PolarStar(Graphics& graphics) : current_level_(1) {
...
void PolarStar::startFire(units::Game player_x, 
  ...
  // Create out projectile using bullet_x, bullet_y
  if (!projectile_a_) {
...
void PolarStar::initializeSprites(Graphics& graphics) {
  for (units::GunLevel gun_level = 0; gun_level < units::kMaxGunLevel; ++gun_level) {
    horizontal_projectiles_[gun_level].reset(new Sprite(graphics,
                                            "Bullet",
                                            units::tileToPixel(kHorizontalProjectileSourceXs[gun_level]),
                                            units::tileToPixel(kProjectileSourceYs[gun_level]),
                                            units::tileToPixel(1),
                                            units::tileToPixel(1)));
    vertical_projectiles_[gun_level].reset(new Sprite(graphics,
                                          "Bullet",
...
void PolarStar::initializeSprite(Graphics& graphics, const SpriteState& sprite_state) {
  ...
  sprite_map_[sprite_state] = boost::shared_ptr<Sprite>(new Sprite(
      graphics, 
      kSpritePath,
...
bool PolarStar::Projectile::update(units::MS elapsed_time, const Map& map, ParticleTools& particle_tools) {
  ...
  for (size_t i = 0; i < colliding_tiles.size(); ++i) {
    if (colliding_tiles[i].tile_type == Map::WALL_TILE) {
      const Rectangle tile_rectangle(units::tileToGame(colliding_tiles[i].col),
