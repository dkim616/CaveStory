...
void MapCollidable::updateX(const CollisionRectangle& collision_rectangle,
  ...
  if (delta > 0.0f) {
    ...
    CollisionInfo info = getWallCollisionInfo(map, collision_rectangle.rightCollision(kinematics_x.position,
    ...
    info = getWallCollisionInfo(map, collision_rectangle.leftCollision(kinematics_x.position,
      kinematics_y.position,
      0));
    if (info.collided) {
    ...
    // Check collision in direction of delta
    CollisionInfo info = getWallCollisionInfo(map, collision_rectangle.leftCollision(kinematics_x.position,
      kinematics_y.position,
      delta));
    // React to collision
    if (info.collided) {
    ...
    // Check collision in other direction
    info = getWallCollisionInfo(map, collision_rectangle.rightCollision(kinematics_x.position,
      kinematics_y.position,
      0));
    if (info.collided) {
      kinematics_x.position = units::tileToGame(info.col) - collision_rectangle.boundingBox().right();
...
void MapCollidable::updateY(const CollisionRectangle& collision_rectangle,
  ...
  if (delta > 0.0f) {
    // Check collision in direction of delta
    CollisionInfo info = getWallCollisionInfo(map, collision_rectangle.bottomCollision(kinematics_x.position,
      kinematics_y.position,
      delta));
    // React to collision
    if (info.collided) {
    ...
    // Check collision in other direction
    info = getWallCollisionInfo(map, collision_rectangle.topCollision(kinematics_x.position,
      kinematics_y.position,
      0));
    if (info.collided) {
    ...
    // Check collision in direction of delta
    CollisionInfo info = getWallCollisionInfo(map, collision_rectangle.topCollision(kinematics_x.position,
      kinematics_y.position,
      delta));
    // React to collision
    if (info.collided) {
    ...
    // Check collision in other direction
    info = getWallCollisionInfo(map, collision_rectangle.bottomCollision(kinematics_x.position,
      kinematics_y.position,
      0));
    if (info.collided) {
      kinematics_y.position = units::tileToGame(info.row) - collision_rectangle.boundingBox().bottom();
